...
QR 출근 체크(`POST /api/shifts/{shiftId}/check-in`)가 검증할 **서명된 토큰을 발급**한다. 사장이 매장에 게시할 QR 토큰을 발급한다. 토큰은 `wp{workplaceId}:{epochSeconds}:{HMAC-SHA256 서명}` 형식이며 서명 자체는 검증 시 재계산한다. 발급 시각은 만료·재사용 검증을 위해 매장별로 **서버 메모리에만** 최신 값 1개가 보관된다(재시작 시 초기화). 직원 앱은 이 토큰을 QR로 스캔해 출근 체크에 사용한다.

응답 `200 OK` — `QrTokenResponse` `{ "qrToken": "wp1:1719740400:9f8a..." }`. 오류: `403`(사장 아님 / 타 매장).

**운영 정책 (서버 설정 `qr.max-age-seconds`, 기본값 `60`):**

| 모드 | 설정 | 사장 앱 동작 | 보안 |
| --- | --- | --- | --- |
| 정적 | `0` | 토큰을 1번 발급해 **QR로 인쇄/게시** | 하드웨어 불필요. 단 QR 사진 도용 시 원격 출근 가능 |
| **회전형(기본)** | 양수(기본 `60`초) | qr-token을 주기적으로 폴링해 화면에 **갱신 표시** | 매장 내 물리적 스캔 강제. 사장 기기 화면 필요 |

기본은 **회전형(60초)**이며, 발급 시각이 만료 기준을 넘거나 그 매장에서 더 최근에 발급된 토큰이 있으면(=이미 갱신된 QR) 검증에 실패한다. 정적 모드로 되돌리려면 서버 `qr.max-age-seconds`를 `0`으로 설정한다(앱 코드 변경 불필요).
